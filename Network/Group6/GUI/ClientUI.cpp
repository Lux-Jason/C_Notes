	  ...
	  mutedMenuItem(nullptr)
{
    ...
    connect(Client::getInstance(), &Client::recv_group_disbanded, this, &ClientUI::onRecvGroupDisbanded);
    connect(Client::getInstance(), &Client::deleteUserRes, this, &ClientUI::onRecvDeleteUserRes);
    connect(Client::getInstance(), &Client::assignRes, this, &ClientUI::onRecvAssignRes);
    ...
    disbandAct = new QAction(QString::fromLocal8Bit("Dissolve"), groupMenu);
    joinAct = new QAction(QString::fromLocal8Bit("Add"), groupMenu);
    leaveAct = new QAction(QString::fromLocal8Bit("Leave"), groupMenu);
    ...
    kickAct = new QAction(QString::fromLocal8Bit("Kick"), memberMenu);
    muteAct = new QAction(QString::fromLocal8Bit("Mute"), memberMenu);
    ...
    QAction* unmuteAct = new QAction(QString::fromLocal8Bit("Unmute"), mutedMenu);
...
void ClientUI::updateGroupsList(const QStringList& groupList)
{
	...
	for (int i = 0; i < groupList.size(); ++i)
	{
        ...
        groupName = groupName.replace(QString::fromLocal8Bit("群主"), QString::fromLocal8Bit("Owner"));
        tmpItem->setName(groupName);
		tmpItem->setNewInfoVisible(false);
...
void ClientUI::updateGroupsMemberList(const QStringList &memList)
{
	...
	for (int i = 0; i < memList.size(); ++i)
	{
		...
		UserItemWidget* tmpItem = new UserItemWidget;
        QString tmpName = memList.at(i);
        tmpName = tmpName.remove(QString::fromLocal8Bit("群组成员："));
...
void ClientUI::updateGroupsMutedList(const QStringList &mutedList)
{
	...
	for (int i = 0; i < mutedList.size(); ++i)
	{
        ...
        if (mutedList.at(i).contains(QString::fromLocal8Bit("无禁言成员")))
        {
            tmpItem->setName("None");
        }
		tmpItem->setNewInfoVisible(false);
...
void ClientUI::onLoginSuccess(const QString &name)
{
    ...
    ui.groupBox_2->setTitle(QString::fromLocal8Bit("Online User"));
    this->show();
    isUserList = true;
...
{
    QString qContent = res.data();
    QStringList tmpList = qContent.simplified().split(" ");
    qContent = QString("You have disbanded the group %1").arg(tmpList.last());
    ui.sysEdit->append(qContent);
	ui.sysEdit->append("\n");
	on_allGroupBtn_clicked();
}
...
{
    ui.sysEdit->append(QString::fromLocal8Bit("Successfully joined the group ") + QString::fromLocal8Bit(res.data()));
	ui.sysEdit->append("\n");
}

void ClientUI::onRecvLeaveRes(const std::string &res)
{
    ui.sysEdit->append(QString::fromLocal8Bit("Successfully left the group ") + QString::fromLocal8Bit(res.data()));
	ui.sysEdit->append("\n");
	on_allGroupBtn_clicked();
}
...
void ClientUI::onRecvCreateRes(const bool &state, const std::string &res)
{
	...
	{
		on_allGroupBtn_clicked();
        ui.sysEdit->append(QString::fromLocal8Bit("Successfully created the group ") + qRes);
		ui.sysEdit->append("\n");
	}
	else
	{
        ui.sysEdit->append(QString::fromLocal8Bit("The group name already exists. Please choose another name."));
		ui.sysEdit->append("\n");
	}
}
...
{
	QString qContent = QString::fromLocal8Bit(content.data());
    qContent = qContent.remove(QString::fromLocal8Bit("Group member：")).simplified();
...
void ClientUI::onRecvKickRes(const std::string &res)
{
    ...
    QStringList tmpList = qContent.simplified().split(" ");
    qContent = QString("%1 have been kicked out of the group").arg(tmpList.at(1));
	ui.sysEdit->append(qContent);
	ui.sysEdit->append("\n");
...
void ClientUI::onRecvKicked(const std::string &res)
{
	QString qContent = QString::fromLocal8Bit(res.data());
    QStringList tmpList = qContent.simplified().split(" ");
    QString tmp = QString("You have been kicked out of the group %1").arg(tmpList.at(1));
    ui.sysEdit->append(tmp);
	ui.sysEdit->append("\n");
...
void ClientUI::onRecvMuteRes(const std::string &res)
{
	QString qContent = QString::fromLocal8Bit(res.data());
    QStringList tmpList = qContent.simplified().split(" ");
    qContent = QString("%1 have been muted.").arg(tmpList.at(1));
	ui.sysEdit->append(qContent);
	ui.sysEdit->append("\n");
...
void ClientUI::onRecvUnmuteRes(const std::string &res)
{
	QString qContent = QString::fromLocal8Bit(res.data());
    QStringList tmpList = qContent.simplified().split(" ");
    qContent = QString("%1 have been removed from the mute status.").arg(tmpList.at(1));
	ui.sysEdit->append(qContent);
	ui.sysEdit->append("\n");
...
void ClientUI::onRecvNewMember(const std::string &content)
{
	QString qContent = QString::fromLocal8Bit(content.data());
    QStringList tmpList = qContent.simplified().split(" ");
    QString tmp = QString("User %1 has joined the group %2").arg(tmpList.at(1)).arg(tmpList.last());
    ui.sysEdit->append(tmp);
	ui.sysEdit->append("\n");
...
void ClientUI::onRecvGroupDisbanded(const std::string &content)
{
    QString qContent = QString::fromLocal8Bit(content.data());
    QStringList tmpList = qContent.simplified().split(" ");
    qContent = QString("Group %1 has been disbanded by the group owner").arg(tmpList.at(1));
    ui.sysEdit->append(qContent);
    ui.sysEdit->append("\n");
}

void ClientUI::onRecvDeleteUserRes(const std::string &res)
{
    QString qRes = QString::fromLocal8Bit(res.data());
    ui.sysEdit->append(QString::fromLocal8Bit("删除用户结果：") + qRes);
    ui.sysEdit->append("\n");
}

void ClientUI::onRecvAssignRes(const std::string &res)
{
    QString qRes = QString::fromLocal8Bit(res.data());
    ui.sysEdit->append(QString::fromLocal8Bit("分配用户结果：") + qRes);
    ui.sysEdit->append("\n");
}

void ClientUI::onMutedInfo(const std::string &content)
{
    ui.sysEdit->append("You have been muted by the group owner and are unable to speak.");
    ui.sysEdit->append("\n");
}
...
void ClientUI::on_allGroupBtn_clicked()
{
	...
	ui.chatStkWidget->setCurrentIndex(0);
	currentChatName.clear();
    ui.groupBox_2->setTitle(QString::fromLocal8Bit("All groups"));
...
void ClientUI::on_sendBtn_clicked()
{
    ...
    if (chatStr.isEmpty())
    {
        QMessageBox::information(this, QString::fromLocal8Bit("Information"), QString::fromLocal8Bit("Please Input the content"));
        return;
    }

    if (currentChatName.isEmpty())
    {
        QMessageBox::information(this, QString::fromLocal8Bit("Information"), QString::fromLocal8Bit("Please select the recipient"));
		return;
    }

	if (isUserList)
	{
        saveToTempFile(true, currentChatName, QString::fromLocal8Bit("Me"), chatStr);
		QString htmlStr = QString("<div style=\"margin:5px; padding:8px; color:#468b58; display:inline-block;\">%1</div>")
            .arg(QString::fromLocal8Bit("Me：") + chatStr);
	...
	else
	{
		...
		else
		{
            QMessageBox::information(this, QString::fromLocal8Bit("Information"),
                QString::fromLocal8Bit("You are not a member of this group. Please join first."));
		}
	}
}
