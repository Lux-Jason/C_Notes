 ...
 */
 /* Those lines marked by [%] are added by Black White
    for more readability.
    Amended by Lux Jason in MSVC environment
  */
#include "DES_c.h"
...
int des_init(int mode)
{
    ...
    {  /* 8个s盒, 每个s盒有6位(64种变化)输入, 每个s盒输出4位分布到32位即1个long中 */
        return 0;
    }
    sbox_output_perm_table_init();
    kn = (unsigned char(*)[8])malloc(sizeof(char) * 16 * 8);
    /* 16轮, 每轮48位密钥分成8字节, 每个字节存放6位密钥信息(左侧2位不用) */
    if (kn == NULL)
    {
    ...
    /* 16组, 每组有4位(16种变化)输入, 4位输出分布到64位即8字节中 */
    if (iperm == NULL)
    {
        free((char*)sbox_output_perm_table);
        free((char*)kn);
    ...
    for (i = 0; i < 16; i++) /* [%] i=SectionNum, each section contains 4 bits; */ /* each input nibble position */
    {
        ...
        for (j = 0; j < 16; j++) /* [%] j=SectionValue varying from 0 to 15; */ /* each possible input nibble */
        {
            ...
            for (j_bit = 0; j_bit < 4; j_bit++)
            {
                ...
                target_bit_num = k;
                perm[i][j][target_bit_num / 8] |= 1 << (7 - target_bit_num % 8);
            }
#if 0
            /* k is the possible target bit for each bit in j */
